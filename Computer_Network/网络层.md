...
- 本地地址或专用地址（`private address`）：内部网络使用的地址，不唯一
  ...
  互联网中的路由器不转发目的地址是专用地址的数据报

- 采用专用IP地址的互连网络称为专用互联网或本地互联网

- 虚拟专用网：利用公用的互联网作为专用网之间的通信载体

  需要专门的硬件和软件并进行配置：连到公网的路由器、路由器协议、加密协议

  <img src="VPN.png" alt="image-20210531165956395" style="zoom:100%;" />

## 网络地址转换（`Network Address Translation, NAT`）

- NAT路由器：将本地地址转换为全球IP地址
- 静态转换：本地地址与全球地址一对一转换，地址可以作为服务器地址
- 动态转换：随机转换
- 网络地址与端口号转换（`Netword Address and Port Translation, NAPT`）
